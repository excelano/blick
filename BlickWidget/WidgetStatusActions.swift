...
struct WidgetStatusActions: Sendable {
    ...
    /// keeping Blick's presence session alive so Graph honors the override.
    ...
    func applyPresence(_ presence: Presence) async throws {
        let wasOutOfOffice = BlickSnapshot.loadFromAppGroup()?.isOutOfOffice == true
        ...
        do {
            ...
            BlickSnapshot.patchAndReload(presence: result.effective, isOutOfOffice: false)
        ...
        do {
            if on {
                try await core.enableAutomaticReplies(
                    defaultMessage: BlickSnapshot.defaultOutOfOfficeMessage
                )
            } else {
                try await core.disableAutomaticReplies()
            }
            let presence = BlickSnapshot.loadFromAppGroup()?.presence ?? .unknown
            BlickSnapshot.patchAndReload(presence: presence, isOutOfOffice: on)
        } catch {
