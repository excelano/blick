...
func widgetEffectiveConfig() -> (clientID: String, authority: String) {
    let defaults = UserDefaults(suiteName: BlickSnapshot.appGroupIdentifier)
    let clientID = defaults?.string(forKey: BlickSnapshot.effectiveClientIDKey)
        ?? PublishedConfig.clientID
    let authority = defaults?.string(forKey: BlickSnapshot.effectiveAuthorityKey)
