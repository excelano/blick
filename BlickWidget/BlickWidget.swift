...
struct BlickEntry: TimelineEntry {
    let date: Date
    let snapshot: BlickSnapshot?
}

struct BlickProvider: TimelineProvider {
    func placeholder(in context: Context) -> BlickEntry {
        BlickEntry(date: Date(), snapshot: previewSnapshot)
    }

    func getSnapshot(in context: Context, completion: @escaping (BlickEntry) -> Void) {
        completion(BlickEntry(date: Date(), snapshot: BlickSnapshot.loadFromAppGroup() ?? previewSnapshot))
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<BlickEntry>) -> Void) {
        Task {
            ...
            let entries = dates.map { BlickEntry(date: $0, snapshot: snapshot) }
    ...
    private func liveSnapshot() async -> BlickSnapshot? {
        let cached = BlickSnapshot.loadFromAppGroup()
    ...
    private var previewSnapshot: BlickSnapshot {
        BlickSnapshot(
            updatedAt: Date(),
...
struct BlickWidgetEntryView: View {
    var entry: BlickProvider.Entry
...
struct BlickWidget: Widget {
    let kind: String = "CheckInWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: BlickProvider()) { entry in
            BlickWidgetEntryView(entry: entry)
...
#Preview(as: .systemMedium) {
    ...
    BlickEntry(date: .now, snapshot: nil)
}
