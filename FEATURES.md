...
| App-icon badge showing unread items | iOS app-icon badge updated to `unread emails + unread chats` after every refresh and after every local mark-read. Meetings are intentionally excluded — they're scheduled, not items to triage. Requests notification permission (`.badge` only) on first use; silently no-ops if denied. |
...
| Open, save, or share an email attachment | Preview sheet header lists each real attachment (paperclip + filename, inline signature logos and tracking pixels filtered out via Klartext's `userFacing` check). Tap one to download it on demand and open it in Quick Look, whose Share action covers Save to Files and open-in-another-app. Cloud-link (`referenceAttachment`) and embedded-message (`itemAttachment`) attachments carry no bytes, so they show a brief "couldn't open" note instead. Bytes are fetched to the device and handed only to Apple's Quick Look / share sheet — no new Graph scope, no new destination. |
...
| Read unread email, chat, or combined message count by voice or Shortcut | "How many unread emails in Blick" (also chats, and a combined "unread messages") routes through `BlickCountIntent`. Returns an integer plus a spoken phrase. |
| Hear how many meetings remain today by voice or Shortcut | "How many more meetings today in Blick" routes through `BlickCountIntent`. Counts the current-or-next meeting plus the rest of today. |
| Hear a work-day overview, and have your unread read aloud, by voice or Shortcut | "What's my Blick" (also "Catch me up in Blick", "Show me my Blick", "What's today's Blick", and similar) routes through `WorkdaySummaryIntent`. It opens with the headline — next meeting plus unread-message counts — then offers progressive disclosure on spoken yes/no gates: first who the unread chats and emails are from; then, on "Want me to read them?", it reads the unread queue aloud (chats first, then emails, each as "Teams/Email from {sender}: {preview}"), capped at 5 items with a "N more unread" tail; then, on "Want the meeting times?", how many meetings remain and each by name and time. A plain "What's my Blick" stays a one-line answer unless you ask to go deeper. The read is one spoken block rather than an item-by-item walk, because App Intents identifies each prompt by source call site, so a dynamic per-item loop collapses and repeats. Disclosure and read-aloud are iPhone Siri only; the watch overview stays single-shot, since its headless snapshot can't carry an interactive follow-up. |
...
| Read an email or Teams chat on the wrist | Tap the email or chat count chip → a list of unread mail / unread chats (3-line rows) → tap one to open a reader. The reader shows the snapshot preview instantly, then the phone relays the full body over WatchConnectivity. Opening marks it read. Out of phone range it keeps the preview with an "open on iPhone" note. The watch still holds no token and makes no Graph call. |
