...
struct WatchStatusProvider: TimelineProvider {
    ...
    private func load() -> BlickSnapshot? {
        BlickSnapshot.loadFromAppGroup(suite: BlickSnapshot.watchAppGroupIdentifier)
    }
}
...
struct BlickWatchCornerWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: WatchStatusProvider()) { entry in
            WatchCornerView(entry: entry)
                ...
                .widgetURL(URL(string: "blick://open"))
        }
...
struct WatchRectangularView: View {
    ...
    private func meetingLayout(meeting: SnapshotMeeting, snapshot: BlickSnapshot) -> some View {
    ...
    private func noMeetingLayout(snapshot: BlickSnapshot) -> some View {
    ...
    private func presenceGlyph(for snapshot: BlickSnapshot) -> some View {
...
struct BlickWatchRectangularWidget: Widget {
...
struct BlickWatchCircularWidget: Widget {
...
struct BlickWatchInlineWidget: Widget {
