...
final class WatchSessionReceiver: NSObject {
    ...
    private(set) var snapshot: BlickSnapshot?
    ...
    override init() {
        ...
        if DemoMode.isActive {
            ...
            demo.saveToAppGroup(suite: BlickSnapshot.watchAppGroupIdentifier)
            return
        }
        #endif
        // Load any previously-pushed snapshot so the glance has something
        // to show on cold launch before a fresh push arrives.
        let initial = BlickSnapshot.loadFromAppGroup(suite: BlickSnapshot.watchAppGroupIdentifier)
    ...
    fileprivate func applyIncoming(_ context: [String: Any]) {
        ...
        guard let decoded = try? JSONDecoder().decode(BlickSnapshot.self, from: data) else {
            logger.error("applyIncoming: failed to decode snapshot")
            return
        }
        snapshot = decoded
        decoded.saveToAppGroup(suite: BlickSnapshot.watchAppGroupIdentifier)
