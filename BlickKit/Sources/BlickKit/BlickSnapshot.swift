...
/// Snapshot of Blick state written to the App Group on every refresh,
...
///
/// Lives in BlickKit so the app, the widget extension, and any future
...
public struct BlickSnapshot: Codable {
    ...
    public func settingStatus(presence: Presence, isOutOfOffice: Bool) -> BlickSnapshot {
        BlickSnapshot(
            updatedAt: updatedAt,
    ...
    public static func loadFromAppGroup(suite: String = appGroupIdentifier) -> BlickSnapshot? {
        guard let defaults = UserDefaults(suiteName: suite),
              let data = defaults.data(forKey: userDefaultsKey) else { return nil }
        return try? JSONDecoder().decode(BlickSnapshot.self, from: data)
    }
