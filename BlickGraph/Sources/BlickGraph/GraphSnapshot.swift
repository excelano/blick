...
/// Assembles a `BlickSnapshot` directly from Graph, so the widget extension
...
public extension GraphCore {
    ...
    func fetchSnapshot() async throws -> BlickSnapshot {
    ...
    private func fetchTodayMeetings() async throws -> [BlickKit.SnapshotMeeting] {
        ...
        return data.value
            ...
            .map { event -> BlickKit.SnapshotMeeting in
                BlickKit.SnapshotMeeting(
