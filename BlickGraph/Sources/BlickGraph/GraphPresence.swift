...
public extension GraphCore {
    ...
    /// Tear down Blick's presence session immediately, rather than waiting
    ...
    /// Register Blick as an active presence-session source so the user's
    ...
    /// (≈30 min), so it never lapses while Blick is in use. `.offline` is not
    ...
    /// (`alwaysEnabled`, `scheduled`) reads as "out of office is on" — Blick
    /// doesn't model scheduled-with-dates; the user manages dates in Outlook
    /// web and Blick shows on/off.
...
/// registers Blick as an active presence source. Distinct from the
...
/// Subset of `/me/mailboxSettings/automaticRepliesSetting` Blick cares about.
