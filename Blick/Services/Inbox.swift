...
final class Inbox {
    ...
    private(set) var summary: BlickSummary?
    ...
    // MARK: - Meeting store
    //
    // Every Meeting Blick knows about lives in `meetingsById`. The four index
    ...
    /// Watch-side relay. Set by `BlickApp.init` after Inbox is built;
    ...
    /// `BlickSnapshot.patchAndReload(...)` updates the App Group.
    ...
    func refresh(fetchAllEmails: Bool = false) async {
        ...
        summary = BlickSummary(emails: emailsResult.emails,
                                 chats: chats,
    ...
    private func publishStatusSnapshot() {
        ...
        BlickSnapshot.reloadStatusSurfaces()
    ...
    //
    // The counts the App Intents speak back. Each reads Blick's in-memory summary
    ...
    /// Every Meeting Blick knows about — today's window, invite
    ...
    /// Re-up Blick's presence session as a pure "I'm here" heartbeat
    ...
    /// on every refresh so it never has a chance to lapse while Blick
    /// is in active use.
    ...
    /// rule lives in `GraphCore`; Blick just shows on/off (the user
    ...
    func setOutOfOffice(_ on: Bool) async -> Bool {
        ...
        do {
            if on {
                try await graphClient.enableAutomaticReplies(defaultMessage: BlickSnapshot.defaultOutOfOfficeMessage)
            } else {
    ...
    /// order so the user sees it back in Blick without having to
    ...
    /// shows up in Blick again. Re-fetches the summary because the
    ...
    /// items that were already read resurface in Blick. Same shape as
...
/// Optimistic: drops the row immediately, restores it (in received-time
    ...
    /// desktop, another mobile client) shows up in Blick again.
...
extension Inbox {
    ...
    func applyPresence(_ presence: Presence) async throws {
        ...
        BlickSnapshot.patchAndReload(presence: currentPresence, isOutOfOffice: isOutOfOffice)
