...
final class AuthService {
    ...
    private func configureMSAL() {
        ...
        if let shared = UserDefaults(suiteName: BlickSnapshot.appGroupIdentifier) {
            shared.set(Constants.effectiveClientID, forKey: BlickSnapshot.effectiveClientIDKey)
            shared.set(Constants.effectiveAuthority, forKey: BlickSnapshot.effectiveAuthorityKey)
        }
    ...
    func signIn(enableTeams: Bool) async throws -> String {
        ...
        do {
            ...
            // Blick before any user in the tenant can sign in. MSAL
