...
/// Phone-side WatchConnectivity link to the Blick watch app. Pushes the
...
/// `BlickSnapshot` (phone → watch) and a small action dictionary
...
final class PhoneConnectivity: NSObject {
    ...
    func push(_ snapshot: BlickSnapshot) {
        guard WCSession.isSupported() else { return }
        let session = WCSession.default
    ...
    /// Used after `BlickSnapshot.patchAndReload(...)` updates the App
    ...
    func pushFromAppGroup() {
        guard let snapshot = BlickSnapshot.loadFromAppGroup() else { return }
        push(snapshot)
    }
